...
export default function NutricionScreen() {
  ...
  const colores = { fondo: esOscuro ? '#000000' : '#f2f2f7', tarjeta: esOscuro ? '#1c1c1e' : '#ffffff', texto: esOscuro ? '#ffffff' : '#1c1c1e', subtexto: esOscuro ? '#8e8e93' : '#666', borde: esOscuro ? '#2c2c2e' : '#eee', inputBg: esOscuro ? '#2c2c2e' : '#f9f9f9', primario: '#007AFF' };
  ...
  useFocusEffect(useCallback(() => { Promise.all([cargarDiario(), calcularMetasDiarias()]); }, []));

  const calcularMetasDiarias = async () => {
    try {
      const [{ data: perfilData }, { data: medicionData }] = await Promise.all([
        supabase.from('perfil').select('*').order('created_at', { ascending: false }).limit(1),
        supabase.from('mediciones').select('peso, grasa_porc').order('fecha', { ascending: false }).limit(1)
      ]);
      ...
      if (usuario.meta_kcal && parseFloat(usuario.meta_kcal) > 0) {
        ...
        setEsDiaEntreno(calendarioData?.estado === 'completado' || (!calendarioData && usuario.nivel_actividad !== 'sedentario'));
        return;
      }

      const peso = medicionData?.[0]?.peso || 78, grasaPorcentaje = medicionData?.[0]?.grasa_porc || 20, actividad = usuario.nivel_actividad || 'moderado';
      ...
      else if (planData?.[0]) { const rutinaDia = planData[0].datos_semana[hoyNombre]; if (rutinaDia?.titulo !== 'Descanso' && rutinaDia?.ejercicios?.length > 0) entrenoHoy = true; }
      setEsDiaEntreno(entrenoHoy);

      const masaMagra = peso - (peso * grasaPorcentaje / 100);
      const tmb = 370 + (21.6 * masaMagra);
      const factores = { sedentario: 1.2, ligero: 1.375, moderado: 1.55, activo: 1.725, muy_activo: 1.9 };
      let objetivoKcal = Math.round(tmb * (factores[actividad] || 1.55));
      ...
      objetivoKcal += entrenoHoy ? 200 : -200;

      const protes = Math.round(peso * 2.2), grasas = Math.round(peso * 0.9);
      const carbos = Math.max(0, Math.round((objetivoKcal - (protes * 4) - (grasas * 9)) / 4));
  ...
  const cargarDiario = async () => {
    const hoy = new Date();
    const { data } = await supabase.from('comidas').select('*').gte('created_at', new Date(Date.UTC(hoy.getFullYear(), hoy.getMonth(), hoy.getDate())).toISOString()).order('created_at', { ascending: false });
    if (data) { setComidasHoy(data); setResumen(data.reduce((acc, i) => ({ kcal: acc.kcal + (i.calorias || 0), protes: acc.protes + (i.proteinas || 0), carbos: acc.carbos + (i.carbos || 0), grasas: acc.grasas + (i.grasas || 0) }), { kcal: 0, protes: 0, carbos: 0, grasas: 0 })); }
  };
  ...
  const analizarConIA = async (uri) => {
    ...
    try {
      ...
      const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({ messages: [{ role: "user", content: [{ type: "text", text: 'Analiza esta imagen de comida. Estima ingredientes, peso total en gramos, nombre del plato y macros por 100g. Responde ÚNICAMENTE con JSON: { "nombre": "Nombre", "explicacion": "Texto", "peso_estimado_g": 0, "kcal_100g": 0, "p_100g": 0, "c_100g": 0, "f_100g": 0 }' }, { type: "image_url", image_url: { url: `data:image/jpeg;base64,${base64}` } }] }], model: "llama-3.2-11b-vision-preview", temperature: 0.1, max_tokens: 1000 })
      });
      const jsonResponse = await response.json();
      if (jsonResponse.error) throw new Error(jsonResponse.error.message);
      const data = JSON.parse(jsonResponse.choices[0]?.message?.content?.match(/\{[\s\S]*\}/)?.[0] || "{}");
      prepararEdicion({ nombre: data.nombre, descripcion: data.explicacion, cantidad: data.peso_estimado_g, macros_100g: { kcal: data.kcal_100g, p: data.p_100g, c: data.c_100g, f: data.f_100g } });
    } catch (e) { Alert.alert("Error IA", "No pude identificar el alimento."); }
  ...
  const recalcularMacrosPorNombre = async () => {
    ...
    try {
      const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        method: "POST", headers: { "Authorization": `Bearer ${GROQ_API_KEY}`, "Content-Type": "application/json" },
        body: JSON.stringify({ messages: [{ role: "system", content: "Eres nutricionista experto. Responde solo JSON." }, { role: "user", content: `Info nutricional por 100g de "${alimentoTemp.nombre}". JSON: { "kcal_100g": 0, "p_100g": 0, "c_100g": 0, "f_100g": 0 }` }], model: "llama-3.3-70b-versatile", temperature: 0.1, max_tokens: 500 })
      });
      const data = JSON.parse((await response.json()).choices[0]?.message?.content?.match(/\{[\s\S]*\}/)?.[0] || "{}");
      const nuevosMacrosBase = { kcal: data.kcal_100g, p: data.p_100g, c: data.c_100g, f: data.f_100g };
      setAlimentoTemp(prev => ({ ...prev, macros_100g: nuevosMacrosBase, macros_finales: calcularMacros(nuevosMacrosBase, parseFloat(prev.cantidad) || 100) }));
      Alert.alert("Actualizado", `Macros actualizados para: ${alimentoTemp.nombre}`);
    } catch (e) { Alert.alert("Error", "No se pudo recalcular."); }
    finally { setCargandoMacros(false); }
  };

  const buscarProductoAPI = async (codigo) => {
    setMostrarEscaner(false); setCargando(true);
    try {
      const data = await (await fetch(`https://world.openfoodfacts.org/api/v0/product/${codigo}.json`)).json();
      if (data.status === 1) {
        const p = data.product, qty = p.serving_quantity ? parseFloat(p.serving_quantity) : 100;
        prepararEdicion({ nombre: p.product_name || "Producto", cantidad: qty, peso_porcion: p.serving_quantity ? parseFloat(p.serving_quantity) : null, macros_100g: { kcal: p.nutriments["energy-kcal_100g"] || 0, p: p.nutriments.proteins_100g || 0, c: p.nutriments.carbohydrates_100g || 0, f: p.nutriments.fat_100g || 0 } });
  ...
  const cargarRecientes = async () => {
    setModoBusqueda('recientes'); setTextoBusqueda('');
  ...
  const buscarEnHistorial = async (texto) => {
    ...
    try { const { data } = await supabase.from('comidas').select('nombre, datos_base, peso_porcion').textSearch('nombre', `'${texto}'`, { config: 'spanish', type: 'websearch' }).limit(15); if (data?.length > 0) { filtrarYSetearResultados(data); return; } } catch (e) { }
  ...
  const filtrarYSetearResultados = (data) => {
    ...
    data.forEach(i => { const n = i.nombre.toLowerCase().trim(); if (!vistos.has(n) && i.datos_base) { vistos.add(n); unicos.push(i); } });
    setResultadosBusqueda(unicos.slice(0, 15));
  };

  const seleccionarDelHistorial = (item) => { setModalBuscarVisible(false); prepararEdicion({ nombre: item.nombre, macros_100g: item.datos_base || { kcal: 0, p: 0, c: 0, f: 0 }, cantidad: 100, peso_porcion: item.peso_porcion }); };
  ...
  const prepararEdicion = (datosBase, esEdicion = false, id = null) => {
    ...
    setAlimentoTemp({ id, nombre: datosBase.nombre, descripcion: datosBase.descripcion || "", cantidad: datosBase.cantidad || 100, peso_porcion: datosBase.peso_porcion || '', macros_100g: macros, macros_finales: calcularMacros(macros, datosBase.cantidad || 100) });
    setModalEditVisible(true);
  };

  const calcularMacros = (base, gramos) => { if (!base) return { kcal: 0, p: 0, c: 0, f: 0 }; const r = gramos / 100; return { kcal: (base.kcal * r).toFixed(0), p: (base.p * r).toFixed(1), c: (base.c * r).toFixed(1), f: (base.f * r).toFixed(1) }; };

  const actualizarTemp = (campo, valor) => {
    if (campo === 'nombre' || campo === 'descripcion') { setAlimentoTemp({ ...alimentoTemp, [campo]: valor }); return; }
    const valorNum = valor.replace(',', '.');
    if (campo === 'peso_porcion') { setAlimentoTemp({ ...alimentoTemp, peso_porcion: valor }); return; }
    const nuevo = { ...alimentoTemp, [campo]: valor };
    const macros = nuevo.macros_100g || { kcal: 0, p: 0, c: 0, f: 0 };
    if (!isNaN(parseFloat(valorNum)) || valorNum === '' || valorNum === '.') {
      if (campo === 'cantidad') nuevo.macros_finales = calcularMacros(macros, parseFloat(valorNum) || 0);
      if (campo.startsWith('base_')) { const k = campo.replace('base_', ''); nuevo.macros_100g = { ...nuevo.macros_100g, [k]: valor }; nuevo.macros_finales = calcularMacros({ ...nuevo.macros_100g, [k]: parseFloat(valorNum) || 0 }, parseFloat(nuevo.cantidad) || 0); }
    }
    setAlimentoTemp(nuevo);
  };

  const usarPorcion = () => { const p = parseFloat(alimentoTemp.peso_porcion?.toString().replace(',', '.')); if (p > 0) actualizarTemp('cantidad', p.toString()); };

  const guardarEnDB = async () => {
    const { nombre, cantidad, macros_100g, macros_finales, id, peso_porcion } = alimentoTemp;
    const cleanFloat = (val) => parseFloat(typeof val === 'string' ? val.replace(',', '.') : val) || 0;
    const d = { nombre, cantidad: cleanFloat(cantidad), peso_porcion: cleanFloat(peso_porcion), unidad: 'g', datos_base: { kcal: cleanFloat(macros_100g.kcal), p: cleanFloat(macros_100g.p), c: cleanFloat(macros_100g.c), f: cleanFloat(macros_100g.f) }, calorias: parseFloat(macros_finales.kcal), proteinas: parseFloat(macros_finales.p), carbos: parseFloat(macros_finales.c), grasas: parseFloat(macros_finales.f) };
    const { error } = id ? await supabase.from('comidas').update(d).eq('id', id) : await supabase.from('comidas').insert(d);
    if (error) Alert.alert("Error al guardar", error.message);
    else { setModalEditVisible(false); cargarDiario(); }
  };

  const borrarComida = (id) => Alert.alert("Borrar", "¿Seguro?", [{ text: "Cancelar" }, { text: "Sí", style: "destructive", onPress: async () => { await supabase.from('comidas').delete().eq('id', id); cargarDiario(); } }]);
  ...
  const InputMacro = ({ label, val, onChange }) => <View style={{ flex: 1, alignItems: 'center' }}><Text style={{ fontSize: 10, color: colores.subtexto }}>{label}</Text><TextInput style={[styles.inputSmall, { borderColor: colores.borde, color: colores.texto }]} keyboardType="decimal-pad" value={val?.toString()} onChangeText={onChange} placeholder="0" placeholderTextColor={colores.subtexto} /></View>;
  const MacroBadge = ({ val, target, label, color }) => <View style={{ alignItems: 'center' }}><Text style={{ fontSize: 18, fontWeight: '900', color }}>{val}</Text><Text style={{ fontSize: 10, color: colores.subtexto }}> / {target}g</Text><Text style={{ fontSize: 12, color: colores.texto, fontWeight: '600' }}>{label}</Text></View>;
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colores.fondo }]}>
      <View style={[styles.headerResumen, { backgroundColor: colores.tarjeta }]}>
        <View style={styles.topRow}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, flex: 1 }}><Text style={[styles.tituloHeader, { color: colores.texto }]}>Diario de Hoy</Text><TouchableOpacity onPress={() => setModalInfoVisible(true)}><Ionicons name="information-circle-outline" size={24} color={colores.primario} /></TouchableOpacity></View>
          <View style={{ alignItems: 'flex-end' }}>{esDiaEntreno ? <View style={[styles.badgeEntreno, { backgroundColor: '#e3f2fd' }]}><Text style={{ color: '#007AFF', fontWeight: 'bold', fontSize: 12 }}>🏋️ Entreno</Text></View> : <View style={[styles.badgeEntreno, { backgroundColor: esOscuro ? '#2c2c2e' : '#eee' }]}><Text style={{ color: colores.subtexto, fontWeight: 'bold', fontSize: 12 }}>💤 Descanso</Text></View>}<Text style={{ fontSize: 10, color: colores.subtexto, marginTop: 2 }}>Nivel: {nivelDetectado}</Text></View>
        </View>
      ...
      <View style={{ flex: 1, paddingBottom: rutinaActiva ? 90 : 0 }}>
        ...
        <View style={[styles.footer, { backgroundColor: colores.tarjeta }]}><BotonFooter icon="camera" text="Foto IA" color="#007AFF" onPress={elegirFoto} /><BotonFooter icon="barcode-outline" text="Escanear" color="#333" onPress={() => setMostrarEscaner(true)} /><BotonFooter icon="create-outline" text="Manual" color="#28a745" onPress={() => { cargarRecientes(); setModalBuscarVisible(true); }} /></View>
      </View>
      ...
      <Modal visible={modalEditVisible} transparent animationType="fade">
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: colores.tarjeta }]}>
            {alimentoTemp?.descripcion ? <View style={{ marginBottom: 15 }}><Text style={{ fontSize: 12, color: colores.primario, fontWeight: 'bold', marginBottom: 5 }}>Análisis de la IA (Puedes corregir):</Text><TextInput style={{ backgroundColor: colores.inputBg, color: colores.texto, borderRadius: 8, padding: 10, height: 80, textAlignVertical: 'top', borderWidth: 1, borderColor: colores.borde }} multiline value={alimentoTemp?.descripcion} onChangeText={(t) => actualizarTemp('descripcion', t)} /></View> : null}
            <View style={{ flexDirection: 'row', alignItems: 'center', borderBottomWidth: 1, borderColor: colores.borde, marginBottom: 15, paddingBottom: 5 }}><TextInput style={[styles.modalTitleEdit, { color: colores.texto, marginBottom: 0, flex: 1 }]} value={alimentoTemp?.nombre} onChangeText={(t) => actualizarTemp('nombre', t)} /><Ionicons name="pencil" size={18} color={colores.subtexto} style={{ marginLeft: 10 }} /></View>
            ...
            {alimentoTemp?.peso_porcion && parseFloat(alimentoTemp.peso_porcion) > 0 && <TouchableOpacity style={{ backgroundColor: '#e3f2fd', padding: 8, borderRadius: 8, alignSelf: 'center', marginBottom: 15 }} onPress={usarPorcion}><Text style={{ color: '#007AFF', fontWeight: 'bold', fontSize: 12 }}>Usar 1 unidad ({alimentoTemp.peso_porcion}g)</Text></TouchableOpacity>}
            <View style={[styles.macrosPreview, { backgroundColor: esOscuro ? '#2c2c2e' : '#f0f8ff', padding: 15, marginBottom: 15 }]}><Text style={{ fontWeight: 'bold', color: colores.texto, marginBottom: 5, textAlign: 'center' }}>Valores para {alimentoTemp?.cantidad || 0}g:</Text><View style={{ flexDirection: 'row', justifyContent: 'space-around' }}><Text style={{ color: colores.texto, fontSize: 12 }}><Text style={{ fontWeight: 'bold' }}>{alimentoTemp?.macros_finales.kcal}</Text> Kcal</Text><Text style={{ color: colores.texto, fontSize: 12 }}><Text style={{ fontWeight: 'bold' }}>{alimentoTemp?.macros_finales.p}</Text> P</Text><Text style={{ color: colores.texto, fontSize: 12 }}><Text style={{ fontWeight: 'bold' }}>{alimentoTemp?.macros_finales.c}</Text> C</Text><Text style={{ color: colores.texto, fontSize: 12 }}><Text style={{ fontWeight: 'bold' }}>{alimentoTemp?.macros_finales.f}</Text> G</Text></View></View>
            <Text style={[styles.labelSmall, { color: colores.subtexto, marginTop: 5, textAlign: 'center' }]}>Peso de 1 unidad/porción (g):</Text>
            <TextInput style={[styles.inputSmall, { borderColor: colores.borde, color: colores.texto, marginBottom: 15, width: '50%', alignSelf: 'center' }]} keyboardType="decimal-pad" value={alimentoTemp?.peso_porcion?.toString()} onChangeText={(t) => actualizarTemp('peso_porcion', t)} placeholder="Ej: 150" placeholderTextColor={colores.subtexto} />
